...
def plot_train_acc(result, epochs, model = 'EEGNet'):
    ...
    plt.title(f'Activation function comparison for training data({model})')
    plt.legend()
    # plt.figure()
    plt.savefig('./{}/Comparison_acc_train{}_{}_{}_{}.jpeg'.format(args.folder, model, args.dropout_rate, args.activation, args.elu_alpha))
    # plt.show()
    plt.close()
...
def plot_test_acc(result, epochs, model = 'EEGNet'):
    plt.figure(figsize=(10, 5))
    epochs = range(1, epochs +1)

    plt.plot(epochs, result['test']['relu'], 'r', label = 'relu_test')
    plt.plot(epochs, result['test']['leakyrelu'], 'g', label = 'leakyRelu_test')
    plt.plot(epochs, result['test']['elu'], 'm', label = 'elu_test')

    plt.title(f'Activation function comparison for testing data({model})')
    plt.legend()
    # plt.figure()
    plt.savefig('./{}/Comparison_acc_test{}_{}_{}_{}.jpeg'.format(args.folder, model, args.dropout_rate, args.activation, args.elu_alpha))
    # plt.show()
    plt.close()
...
if __name__ == '__main__':
    ...
    plot_train_acc(result = result, epochs = args.num_epochs, model = args.model_n)
    plot_train_loss(loss_list, args.num_epochs, model = args.model_n)
    plot_test_acc(result = result, epochs = args.num_epochs, model = args.model_n)
